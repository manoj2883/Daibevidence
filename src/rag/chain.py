...
2. Output your answer as a single JSON array, one object per sentence, in order. Each object has \
...
genuinely follows from those excerpts). If a specific clinical claim cannot be attributed to any \
excerpt, do not include it as a sentence at all — never present an unsupported clinical claim, whether \
marked or not.
3. Before the JSON array, compare the source excerpts for genuine contradictions — cases where two \
excerpts report conflicting findings on the same specific claim (not just different topics, and not a \
population difference — that is handled separately). Report this as a JSON preamble, exactly as \
follows, before the sentence array: a line containing exactly "{contra_start}", then a JSON array (or \
"[]" if none), then a line containing exactly "{contra_end}". Each array entry must have exactly these \
keys: "excerpt_indices" (array of the [Excerpt N] numbers involved, e.g. [1, 3]), "claim" (the specific \
point they disagree on), "position_a" (what the first excerpt found), "position_b" (what the other \
found), and "differs_by" (what differs between the studies that could explain the disagreement — e.g. \
population, study design, duration, sample size). If you report a contradiction here, your sentence \
array below MUST present both positions rather than silently picking one. \
Exception: if you are refusing per rule 6 below, skip this preamble and the JSON array entirely and \
...
under rule 3 — population mismatch is a distinct thing from two studies disagreeing.
