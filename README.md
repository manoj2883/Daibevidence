...
3. Four possible outcomes, exposed as `state` — only `out_of_scope` is knowable before generation; the other three depend on which chunks the model actually ends up citing (a chunk can clear the floor and still go unused if the model correctly decides it doesn't answer the question), so the `sources` event carries a cheap *provisional* guess and the `done` event carries the *authoritative* final value (`determine_final_state()`):
   - **`out_of_scope`** — nothing cleared the floor in *either* tier. A `refusal` event is sent with the closest scores found, a description of what the system does cover, and the full score distribution. **No Claude call is made.**
   - **`no_evidence_for_claim`** — the question is in scope (something cleared the floor), but no `evidence`-tier chunk ends up cited by any sentence — only `background` chunks, or nothing at all. The model still gives whatever definitional context the background tier supports, then states plainly that no study in the retrieved literature addresses the specific claim (SYSTEM_PROMPT rule 7) — the interesting refusal, not an error.
   - **`answered_low_confidence`** — an `evidence`-tier chunk is cited, but its score is only marginally above the floor.
